    ...
    <section className="bg-[#F8FAFC] ">
      <div className="max-w-[85%]  py-20 w-full mx-auto">
        <div className="flex justify-between items-center">
          <h3 className="font-bold text-4xl">Most Popular Products</h3>
          <Button className="px-10">View All</Button>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 mt-10">
          {products.slice(8,16).map((product: any, i: number) => (
            <Card
              key={i}
              ...
              <CardFooter>
                <Button onClick={()=>addToCart(product)} className="w-full bg-teal-500 hover:bg-teal-600 text-white">
                  Add To Cart
                </Button>
              </CardFooter>
            </Card>
